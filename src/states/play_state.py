...
class PlayState(BaseState):
    ...
    def update(self, dt: float) -> None:
        ...
        target_x, target_y = self.pseudo_3D.mouse_to_world(self.mouse_x, self.mouse_y)
    ...
    def render(self, screen: py.Surface) -> None:
        ...
        table_infright_sx, _, _ = self.pseudo_3D.world_to_screen(
            self.table.x + self.table.half_width, self.table.y - self.table.half_length, self.table.z)
        self.renderer.render_by_scanlines((table_infleft_sx, table_supleft_sx, table_min_sy, table_max_sy, 160), self.resource_manager.get_texture("PLAY", "table"))
